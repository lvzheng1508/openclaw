...
## 10. 个性化本地修改恢复 (DeepSeek 模型)

如果合并上游 `main` 分支时发生了冲突，或者上游代码覆盖了我们在 `extensions/deepseek/models.ts` 里的自定义修改，请手动恢复以下关于 **DeepSeek V4 (1M 上下文)** 的改动：

**文件：** `extensions/deepseek/models.ts`
...
**需保留/添加的模型定义 (`DEEPSEEK_MODEL_CATALOG` 数组内)：**
```typescript
  {
